...
function parseOneFile(filePath,label){
  try{
    ...
    for(let i=5;i<ar.length;i++){
      const r=ar[i];if(!r||!r[0])continue;
      const v0=String(r[0]).trim();
      if(v0==="Total"||v0==="UNIQUE ORDERS"||v0==="Center Name")break;
      const date=fmtDate(r[0]);if(!date)continue;
    ...
    // Find centre summary rows dynamically (after "UNIQUE ORDERS" or "Center Name")
    const centreRevenue=[];
    for(let i=38;i<ar.length;i++){
      const r=ar[i];if(!r)continue;
      const v0=String(r[0]||"").trim();
      if(v0==="Center Name"||v0==="Centre Name")continue;
      if(v0==="Total"&&centreRevenue.length>0)break;
      if(v0&&v0!=="UNIQUE ORDERS"&&v0!==""&&num(r[2])>0){
        centreRevenue.push({name:v0.replace("New Delhi","Delhi"),adm:num(r[1]),value:num(r[2])});
      }
    }
    ...
    const walkinData=[];
    for(let i=6;i<lf.length;i++){
      const r=lf[i];if(!r||!r[0])continue;
      const v0=String(r[0]).trim();
      if(v0==="Total"||v0==="Date ")break;
      const date=fmtDate(r[0]);if(!date)continue;
    ...
    let totIdx=38;
    for(let i=35;i<Math.min(lf.length,45);i++){
      if(lf[i]&&String(lf[i][0]).trim()==="Total"){totIdx=i;break;}
    }
    const tot=lf[totIdx]||[];
    ...
    return {
      ...
      totals:{revenue:totalGsRev+totalOtherRev,gsRevenue:totalGsRev,otherRevenue:totalOtherRev,
        admissions:totalGsAdm+totalOtherAdm,gsAdmissions:totalGsAdm,otherAdmissions:totalOtherAdm,
