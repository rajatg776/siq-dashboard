...
function computeOverall(months){
  ...
  for(const m of valid){
    ...
    // Build month-wise centre revenue for charts
    const mRow={month:m.label,revenue:m.totals.revenue,admissions:m.totals.admissions,walkins:m.totals.walkins};
    // Centre-wise revenue for this month
    for(const c of m.centreRevenue){
      const key=c.name.toLowerCase().replace(/\s+/g,"");
      mRow[key]=c.value;
      mRow[key+"_adm"]=c.adm;
    }
    // Centre-wise walkins for this month
    if(m.walkinData.length>0){
      const wTotals={delhi:0,patna:0,lucknow:0,prayagraj:0,indore:0};
      for(const w of m.walkinData){for(const k of Object.keys(wTotals))wTotals[k]+=w[k]||0;}
      Object.assign(mRow,wTotals);
    }
  ...
  return {
    label:"Overall",dailyData:[],centreRevenue,walkinData:[],leadFunnelData,
