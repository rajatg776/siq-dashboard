...
export default function Dashboard({data}){
  ...
  if(!cur||!cur.hasData){
    return(<div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'DM Sans',sans-serif",padding:"24px 20px"}}>
      ...
      <div style={{marginTop:24}}>
        <select value={selMonth} onChange={e=>setSelMonth(e.target.value)} style={{background:C.card,color:C.text,border:`1px solid ${C.cardBorder}`,borderRadius:8,padding:"8px 16px",fontSize:14,fontFamily:"inherit"}}>
          {allOptions.map(o=><option key={o.key} value={o.key} disabled={o.disabled}>{o.label}</option>)}
        </select>
      </div>
      <div style={{marginTop:60,textAlign:"center"}}><p style={{color:C.textDim,fontSize:16}}>No data available for this month.</p><p style={{color:C.textDim,fontSize:13}}>Upload an Excel file with the correct sheet structure to see data here.</p></div>
    </div>);
  }

  const{dailyData:rawD,centreRevenue,walkinData:rawW,leadFunnelData,totals}=cur;
  const daily=rawD.map(d=>({...d,date:sD(d.date)}));
  const walkin=rawW.map(d=>({...d,date:sD(d.date)}));
  ...
  const isOverall=selMonth==="overall";
  const mSummary=cur.monthSummary||[];
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",padding:"24px 20px"}}>
      ...
      <div style={{marginBottom:28}}>
        ...
        <p style={{color:C.textDim,fontSize:13,marginLeft:20,marginTop:4}}>
          {isOverall?`${validMonths.length} months aggregated`:`${cur.label}`} • {days} days • <span style={{color:C.green}}>Live from Excel</span> • {months.length} file{months.length>1?"s":""} detected
        </p>
      </div>

      {/* Month-over-Month Trend (only in Overall view) */}
      {isOverall&&mSummary.length>1&&(
        <Cd style={{marginBottom:18}}>
          <Tt sub={`${mSummary.length} months`}>Month-over-Month Trend</Tt>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:18}}>
            <div><p style={{color:C.textDim,fontSize:11,textTransform:"uppercase",letterSpacing:1,margin:"0 0 8px"}}>Revenue</p>
              <ResponsiveContainer width="100%" height={180}><BarChart data={mSummary}><CartesianGrid strokeDasharray="3 3" stroke={C.gridLine} vertical={false}/><XAxis dataKey="month" tick={{fill:C.text,fontSize:11}} tickLine={false} axisLine={false}/><YAxis tick={{fill:C.textDim,fontSize:10}} tickFormatter={fS} tickLine={false} axisLine={false}/><Tooltip formatter={v=>fmt(v)}/><Bar dataKey="revenue" name="Revenue" fill={C.accent} radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
            </div>
            <div><p style={{color:C.textDim,fontSize:11,textTransform:"uppercase",letterSpacing:1,margin:"0 0 8px"}}>Admissions</p>
              <ResponsiveContainer width="100%" height={180}><BarChart data={mSummary}><CartesianGrid strokeDasharray="3 3" stroke={C.gridLine} vertical={false}/><XAxis dataKey="month" tick={{fill:C.text,fontSize:11}} tickLine={false} axisLine={false}/><YAxis tick={{fill:C.textDim,fontSize:10}} tickLine={false} axisLine={false}/><Tooltip/><Bar dataKey="admissions" name="Admissions" fill={C.green} radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
            </div>
            <div><p style={{color:C.textDim,fontSize:11,textTransform:"uppercase",letterSpacing:1,margin:"0 0 8px"}}>Walk-ins</p>
              <ResponsiveContainer width="100%" height={180}><BarChart data={mSummary}><CartesianGrid strokeDasharray="3 3" stroke={C.gridLine} vertical={false}/><XAxis dataKey="month" tick={{fill:C.text,fontSize:11}} tickLine={false} axisLine={false}/><YAxis tick={{fill:C.textDim,fontSize:10}} tickLine={false} axisLine={false}/><Tooltip/><Bar dataKey="walkins" name="Walk-ins" fill={C.amber} radius={[6,6,0,0]}/></BarChart></ResponsiveContainer>
            </div>
          </div>
        </Cd>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,marginBottom:24}}>
        <Kpi label={isOverall?"Total Revenue (All)":"Total Revenue"} value={fmt(totals.revenue)} sub={`Avg ${fmt(avgR)}/day`} color={C.accent} icon="₹"/>
        ...
        <Kpi label="Avg Ticket Size" value={fmt(avgT)} sub={isOverall?`${validMonths.length} months`:`Peak: ${top.date||"-"}`} color={C.purple} icon="◆"/>
      </div>
      ...
      {tab==="revenue"&&(<div style={{display:"grid",gap:18}}>
        ...
        <Cd><Tt>Cumulative Revenue</Tt><ResponsiveContainer width="100%" height={250}><AreaChart data={cum}><defs><linearGradient id="cg" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={C.green} stopOpacity={0.3}/><stop offset="95%" stopColor={C.green} stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke={C.gridLine} vertical={false}/><XAxis dataKey="date" tick={{fill:C.textDim,fontSize:10}} tickLine={false} axisLine={false} interval={Math.max(Math.floor(daily.length/12),1)}/><YAxis tick={{fill:C.textDim,fontSize:10}} tickFormatter={fS} tickLine={false} axisLine={false}/><Tooltip content={<Tp/>}/><Area type="monotone" dataKey="cumRev" name="Cumulative" stroke={C.green} fill="url(#cg)" strokeWidth={2.5} dot={false}/></AreaChart></ResponsiveContainer></Cd>
      ...
      {tab==="leads"&&(<div style={{display:"grid",gap:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
          ...
          <Cd><Tt>Lead Sources by Centre</Tt><ResponsiveContainer width="100%" height={300}><BarChart data={srcBar} barCategoryGap="20%"><CartesianGrid strokeDasharray="3 3" stroke={C.gridLine} vertical={false}/><XAxis dataKey="name" tick={{fill:C.text,fontSize:11}} tickLine={false} axisLine={false}/><YAxis tick={{fill:C.textDim,fontSize:10}} tickLine={false} axisLine={false}/><Tooltip/><Legend wrapperStyle={{fontSize:10}} formatter={v=>SoL[v]||v}/>{srK.map(k=><Bar key={k} dataKey={k} name={k} stackId="s" fill={SoC[k]} radius={k==="Events"?[3,3,0,0]:undefined}/>)}</BarChart></ResponsiveContainer></Cd>
        </div>
